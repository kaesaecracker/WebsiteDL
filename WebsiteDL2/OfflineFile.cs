namespace WebsiteDL
{
    ...
    internal class OfflineFile
    {
        ...
        {
            // TODO port?
            if (uriStr.StartsWith("//", System.StringComparison.Ordinal))
            {
                uriStr = uriStr.Remove(0, 2);
            }

            if (!uriStr.StartsWith("http", System.StringComparison.Ordinal))
            {
                uriStr = "http://" + uriStr;
            }
            ...
            return uri.Scheme + "://" + (uri.Host.StartsWith("www.", System.StringComparison.Ordinal) ? uri.Host.Remove(0, 4) : uri.Host) + uri.PathAndQuery;
        }
    }
}
